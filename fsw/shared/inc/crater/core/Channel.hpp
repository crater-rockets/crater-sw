...
private:
    ...
    Receiver(std::shared_ptr<ReceiverInner> inner, size_t size)
        : inner_(inner),
          size_(size)
    {
    }
...
public:
    Channel()
        : inner_(std::make_shared<ChannelInner>()) {};
    ...
    {
        using RxInner                     = typename Receiver<T>::ReceiverInner;
        std::shared_ptr<RxInner> rx_inner = std::make_shared<RxInner>(crt::collections::CircularBuffer<T>(buf_size));
...
private:
    Sender(std::shared_ptr<typename Channel<T>::ChannelInner> channel)
        : channel_(channel)
    {
    }
